...
class Trainer(object):
    ...
    def __init__(self, data_loader, config, val_loader):
        ...
        self.indicator = False if self.pretrained_model > 0 else True #default 0
        ...
        for epoch in range(start, self.epochs):
            ...
            for i_iter, batch in enumerate(self.data_loader):
                ...
                if self.arch in __BA__:
                    ...
                    c_loss = criterion(preds, [labels, edges])  #preds:list; labels:tensor
            ...
            if f1 > best_f1:
                best_f1 = f1
                torch.save(self.G.state_dict(), osp.join(
                    self.model_save_path, '{}_{}_G.pth'.format(str(epoch), str(round(best_f1, 4)))))
