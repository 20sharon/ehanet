...
class CelebAMaskHQ(Dataset):
    ...
    def preprocess(self):
        ...
        #     img_path = osp.join(self.img_path, str(os.listdir(self.img_path)[i]) +'.jpg')
        #     label_path = osp.join(self.label_path, str(os.listdir(self.img_path)[i]) +'.png')
        ...
        if self.mode:
            self.train_dataset = sorted(os.listdir(self.img_path))
            self.train_label_dataset = sorted(os.listdir(self.label_path))
        else:
            self.test_dataset = sorted(os.listdir(self.img_path), key=lambda x: int(x.split('.')[0]))
            self.test_label_dataset = sorted(os.listdir(self.label_path), key=lambda x: int(x.split('.')[0]))
            # print(self.test_dataset)
        print("preprocess DONE")
    def __getitem__(self, index):
        dataset = self.train_dataset if self.mode == True else self.test_dataset
        label_dataset = self.train_label_dataset if self.mode == True else self.test_label_dataset
        # img_path, label_path = dataset[index]
        img_path = osp.join(self.img_path, dataset[index])
        label_path = osp.join(self.label_path, label_dataset[index])
        osp.join(self.img_path, img_path)
...
class CustomDataLoader:
    def __init__(self, img_path, label_path, image_size, batch_size, num_workers, transform=None, mode=True, shuffle=False):
        self.img_path = img_path
        self.label_path = label_path
